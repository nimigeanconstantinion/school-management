...
//           elmm.style.opacity=1
    ...
    let getToken=async ()=>{
        await dispatch(loginUser({username:eml.current.value,password:pass.current.value}));
    }
    let validateUser = async (e) => {
        e.preventDefault();
        await dispatch(loginUser({username:eml.current.value,password:pass.current.value}));
        let tkn=store.getState().login.retToken.replace("Bearer ","");
        console.log(tkn);
        let api=new Api();
        if(tkn!=""&&tkn!=undefined) {
                console.log("am obtinut tokenul");
                console.log(token);
                let data = await api.getUser(eml.current.value, pass.current.value);
                console.log(data);
                setStudent(data);
                const usr = {};
                usr.id = data.id;
                ...
                let msg="";
                if(eml.current.value==""||pass.current.value==""){
